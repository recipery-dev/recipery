...
export function ShoppingListPage() {
  ...
  const [servingsByRecipe, setServingsByRecipe] = React.useState<
  ...
  const servingsFor = (recipe: Recipe) =>
    servingsByRecipe[recipe.id] ?? recipe.servings ?? 0;
  const setServingsFor = (recipe: Recipe, next: number) =>
    setServingsByRecipe((prev) => ({
      ...prev,
      [recipe.id]: Math.max(1, next),
    }));
  ...
  const lines = buildShoppingList(
    addedRecipes.map((recipe) => ({ recipe, factor: factorFor(recipe) })),
  );
    ...
    return (
      <Empty className="border py-16">
        <EmptyHeader>
          ...
          <EmptyDescription>
            Add recipes from your library — use “Add to shopping list” from a
            recipe’s menu.
          </EmptyDescription>
        </EmptyHeader>
      </Empty>
            ...
            {recipe.servings !== undefined && (
              <div className="flex shrink-0 items-center gap-0.5 rounded-full bg-secondary px-1 py-1">
                <button
                  ...
                  onClick={() =>
                    setServingsFor(recipe, servingsFor(recipe) - 1)
                  }
                  ...
                  <li key={line.key}>
                    <label className="flex cursor-pointer items-start gap-2.5 text-sm">
                      <Checkbox
                        checked={isChecked}
                        onCheckedChange={() =>
                          toggleShoppingListChecked(line.key)
                        }
                        className="mt-0.5"
                      />
                      <span
