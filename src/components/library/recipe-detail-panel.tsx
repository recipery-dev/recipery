  ...
  return (
    <aside className="relative flex h-full w-full flex-col overflow-x-hidden overflow-y-auto p-6">
      ...
      <div className="flex gap-4 pr-8">
        <RecipePhoto title={recipe.title} coverUrl={recipe.coverUrl} className="w-28" />
        <div className="flex min-w-0 flex-col justify-center gap-1.5">
          <h2 className="font-heading text-lg font-bold leading-snug text-balance">{recipe.title}</h2>
          {recipe.source && <p className="truncate text-sm text-muted-foreground">{recipe.source}</p>}
          {totalMin !== undefined && (
          ...
          {(recipe.tags.length > 0 || recipe.difficulty || recipe.cuisine) && (
      ...
      {recipe.description && (
        <p className="mt-4 text-sm leading-relaxed text-muted-foreground">{recipe.description}</p>
      )}
            ...
            {activity.map((a) => (
              <div key={a.profileId} className="flex items-center gap-2">
                <Avatar className="size-6 shrink-0">
                  <AvatarFallback
                    className={cn(a.color, "text-[10px] font-semibold text-white")}
                  >
